...
- refactor: replace Python tools (Jinja2 generator, validator CLI) with native Go implementations
- refactor: eliminate two-stage template pipeline — source-config.yaml now embedded and computed at render time
- feat: add cmd/validate/ (Go port of Python metrics validator)
- feat: add cmd/docgen/ (Go port of Python markdown doc generator)
- ci: remove check-generated-configs job, update validator jobs to use Go
